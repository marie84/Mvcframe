<?php
/**
 * A model for managing Mvcframe modules.
 * 
...
class CMModules extends CObject {
   ...
   * Properties
   */
  private $MvcframeCoreModules = array('CMvcframe', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $MvcframeCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isMvcframeCore']   = in_array($rc->name, $this->MvcframeCoreModules);
      $details['isMvcframeCMF']    = in_array($rc->name, $this->MvcframeCMFModules);
